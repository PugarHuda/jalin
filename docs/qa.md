...
sh contracts/test.sh   # 48 Cairo tests, four of which fork mainnet
```

| Suite | What it establishes |
|---|---|
| Cairo, 48 | Each of the six invariants rejects the plan it exists to reject. Four fork mainnet and run the router against Endur's real vault and AVNU's real exchange, so the integration is tested against the deployed contracts rather than a mock. |
