...
npm run test:e2e  # 388 browser tests, six projects, three engines
...
| Browser, 388 | Every page renders without a console error, passes axe at WCAG 2.1 AA, keeps a visible focus ring under real Tab presses, never scrolls sideways at 390px, and computes every colour pair on the page against the WCAG formula from the tokens on the live document. |
