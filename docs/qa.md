...
sh contracts/test.sh   # 49 Cairo tests, six of which fork mainnet
```

| Suite | What it establishes |
|---|---|
| Cairo, 49 | Each of the six invariants rejects the plan it exists to reject. Six fork mainnet and run the router against Endur's real vault, AVNU's real exchange and Vesu's real STRK lending market — including one plan carrying a swap and a stake in the same invoke — so the integration is tested against the deployed contracts rather than a mock. |
